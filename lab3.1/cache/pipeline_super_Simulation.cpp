...
Memory m;
Cache l1, l2, llc;
int total_hit, total_miss, total_request;
...
void init_memory()
{
    ...
    cc.size = 32 * 1024;
    cc.associativity = 8;
    cc.set_num = cc.size / cc.associativity / 64;
    cc.write_through = 0;
    cc.write_allocate = 1;
    l1.SetConfig(cc);
...
void load_memory()
{
	...
	fread(&m.memory[vadr],1,csize,file);
	fseek(file,dadr,SEEK_SET);
	fread(&m.memory[dvadr],1,dsize,file);
	fseek(file,badr,SEEK_SET);
	fread(&m.memory[gb],1,bsize,file);
	//memset(&memory[gb],0,bsize);
	fseek(file,sdadr,SEEK_SET);
	fread(&m.memory[sdvadr],1,sdsize,file);
	//printf("%d %d %x\n", gb, bsize, &memory[gb]);
		...
		if(strcmp(cmd, "m") == 0)
		{
			...
			for(int i = 0; i < num; ++i)
			{
				...
				if(i % 16 == 0)
					printf("%x: ", s+i);
				int hit, time;
				l1.HandleRequest((uint64_t)(s+i), 1, 1, (char*)&c, hit, time);
				printf("%02x ", c);
				if(i % 16 == 15)
					printf("\n");
			}
...
int main(int argc, char** argv)
{
	...
	while(1)
	{
		...
		cout << endl;
		cout << "Cache hit:             " << total_hit << endl;
		cout << "Cache miss:            " << total_miss << endl;
		cout << "Cache miss rate:       " << (double)total_miss * 100 / total_request << '%' << endl;
		cout << endl;
...
void simulate()
{
    ...
    for(int i = 0; i < l1.config_.set_num; ++i)
    {
    	for(int j = 0; j < l1.config_.associativity; ++j)
    	{
    		if(blocks[i][j].write == 1)
    		{
    			l1.lower_->HandleRequest()
    		}
    	}
    }
#endif
...
void IF()
{
	...
	int hit, time;
#ifdef CREATE_TRACE
	fout << "r " << hex << "0x" << PC << endl;
#endif
	l1.HandleRequest(PC, 4, 1, (char*)&IF_ID_old.inst, hit, time);
	cycles += time - 1;
	if(hit)
		total_hit++;
	else
		total_miss++;
	total_request++;
	//IF_ID_old.inst=*((unsigned int*)&m.memory[PC]);
	IF_ID_old.PC=PC;
#ifdef DEBUG
...
void MEM()
{
	...
	{
#ifdef CREATE_TRACE
		fout << "w " << hex << "0x" << ALU_out << endl;
#endif
		printf("Write: %d, %d, %d ...... ", ALU_out, Reg_Rs2, MemWrite);
		l1.HandleRequest((uint64_t)ALU_out, MemWrite / 8, 0, (char*)&Reg_Rs2, hit, time);
	...
	{
#ifdef CREATE_TRACE
		fout << "r " << hex << "0x" << ALU_out << endl;
#endif	
		l1.HandleRequest((uint64_t)ALU_out, MemRead / 8, 1, (char*)&Mem_read, hit, time);
		printf("Read: %d, %d, %d ...... ", ALU_out, Mem_read, MemRead);
